...
def main():
    param = input('키워드를 입력해주세요 : ')
    print('시작하려는 음식 종류를 입력해주세요.')
    startmenu = input('처음부터 시작하려면 1을 입력해주시면 됩니다. : ')
    lastmenu = input('음식종류의 총 개수를 입력해주세요. : ')
    ...
    try:
        for menu in range(int(startmenu), int(lastmenu) + 1):
            ...
            for num in range(1, 16, 5):
                ...
                # url 검색
                time.sleep(random.uniform(8, 10))  # 8~10초동안 정지
...
def onePageToDf(restaurant_list, menu, page, fname):
    ...
    for i in range(0, 20):
        ...
        try:
            ...
            response = requests.get(restaurant_url)  # 재시도
